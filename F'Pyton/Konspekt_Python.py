...
peremennaya: str = "" # явно задали тип str переменной peremennaya комментарий
...
'qwer' * 3 # 'qwerqwerqwer'
 # Порядок мат. операций **, унарный+, унарный-, *, /, //, %, +, -
'+= 5' # прибавить к переменной 5, *=5

#############################################################
## Логические операции: >, <, >=, <=, ==, !=, not, and, or
# ПОРЯДОК ЛОГИЧЕСКИХ ОПЕРАЦИЙ: (), !, &, || is, is not, in, not in
#############################################################
...
while условие:
    действие
    действие
else:        # когда основное тело цикла перестает работать действие
    действие 
-------------
...
text.startswith(ch[0]) # Возвращает флаг, указывающий на то, начинается ли строка с указанного префикса.
...
' '.join(words) #  слепить список в строку с разделением пробелами
...
def f(*x) #  '*' обозначает неограничееное количество безымянных аргументов пакуются в кортеж
...
exit() #  позвляет не выполнять код, который ниже по тексту или 
...
from os import path #  подсключаем path и:
path.exist(файл для открытия) #  возвращает true если файл существует
-----------------------------------
...
{ch[0]: list(filter(lambda el: el.startswith(ch[0]), args)) for ch in sorted(args)} #  создание словаря
dict.setdefault(ключ, значение) #  создает пару ключ-значениеб возвращает значение
dictionary.setdefault(text.split()[1][0], {}).setdefault(text.split()[0][0], []).append(n_s) #  ключ первая буква второго слова, значение пустой объект. потом в него пишется еще словарь, ключ первая буква первого слова, возвращает значение массив, в него пишется фраза
...
--------------------
filter() # применяет указанную функцию к каждому элементу итерируемого объекта и возвращает итератор с теми обьектами, которые вернули true
# надо распаковывать в список! 
# нельзя пройтись дважды
data=list(filter(lambda x: not x%2, data)) # список четных чисел
----------------------
zip() # возвращает итератор с кортежами из элементов входных данных
...
-----------------------
enumerate() # возвращает список кортежей из индекса и элементов входящих значений(нумерует)
enumerate(['ddd', 'eee', 'fff']) # получаем [(0, 'ddd'), (1, 'eee'), (2, 'fff')]

----------------------------
eval('7 + 6 * 0') # выводит 7
--------------------------------
https://pypi.org/ библиотеки для скачивания
# python main.py - для старта бота
# ctrl C - выход из процесса  в консоли
